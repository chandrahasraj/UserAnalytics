...
class KinesisHelper implements IShardListGetter {
	...
	private static Map<String, ShardInfo> SplitShards = new HashMap<>();
	private static Map<String, ShardInfo> MergeShards = new HashMap<>();
	...
	/**
	 * @param streamName
	 *            Kinesis stream name to interact with.
	 * @param kinesisCredsProvider
	 *            Credentials for authentication with Kinesis.
	 * @param kinesisClientConfig
	 *            Configuration for the Kinesis client.
	 */
	KinesisHelper(final String streamName,
		...
		this.serializedKinesisCredsProvider = SerializationHelper
				.kryoSerializeObject(kinesisCredsProvider);
		this.serializedkinesisClientConfig = SerializationHelper
				.kryoSerializeObject(kinesisClientConfig);
	...
	public ImmutableSortedMap<String, ShardInfo> getShardList() {
		...
		while (true) {
			String lastShard = addTruncatedShardList(spoutShards, out
					.getStreamDescription().getShards());

			// If we have finished processing all the shards, we can stop
			// looping
	...
	/**
	 * @return new instance of AmazonKinesisClient, with parameters supplied by
	 *         whatever was passed to the KinesisHelper constructor.
	 */
	private AmazonKinesisClient makeNewKinesisClient() {
		final AmazonKinesisClient client = new AmazonKinesisClient(
				getKinesisCredsProvider(), getClientConfiguration());
		return client;
	}
	...
	private AWSCredentialsProvider getKinesisCredsProvider() {
		if (kinesisCredsProvider == null) {
			kinesisCredsProvider = (AWSCredentialsProvider) SerializationHelper
					.kryoDeserializeObject(serializedKinesisCredsProvider);
		}
	...
	private ClientConfiguration getClientConfiguration() {
		if (kinesisClientConfig == null) {
			kinesisClientConfig = (ClientConfiguration) SerializationHelper
					.kryoDeserializeObject(serializedkinesisClientConfig);
		}
	...
	private String addTruncatedShardList(
			final Map<String, ShardInfo> spoutShards,
			final List<Shard> streamShards) {
		String currShard = "";
		...
		for (Shard s : streamShards) {
			...
			if (s.getParentShardId() != null
					&& s.getAdjacentParentShardId() != null) {
				// It's a merge. Set both parents of the merge to merge into
				// this shard.
				// spoutShards.get(s.getShardId()).setMergesInto(s.getShardId());
					SplitShards.put(s.getShardId(),
							new ShardInfo(s.getParentShardId()));
					SplitShards.put(s.getShardId(),
							new ShardInfo(s.getAdjacentParentShardId()));
			} else if (s.getParentShardId() != null) {
				// It's a split. Add the current shard to the split list of its
				// parent.
				
				MergeShards.put(s.getShardId(),new ShardInfo(s.getParentShardId()));
			}
		}
