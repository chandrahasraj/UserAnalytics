...
class KinesisShardGetter implements IShardGetter {
    ...
    public Records getNext(int maxNumberOfRecords)
        ...
        for (Record rec : result.getRecords()) {
            records.add(rec);
            bufferSize.addAndGet(rec.getData().array().length);
        ...
        LOG.debug(this + " fetched " + result.getRecords().size() + " records from Kinesis (requested "
                + maxNumberOfRecords + " ).");
		/*LOG.info(this + " fetched " + result.getRecords().size()
				+ " with byte size:" + bufferSize.get()
				+ " bytes, records from Kinesis (requested "
				+ maxNumberOfRecords + " ).");*/
		bufferSize.set(0l);
